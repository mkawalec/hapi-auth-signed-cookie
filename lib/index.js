...
function getSignature(password, session) {
  var toSign = R.omit([ '_signature' ], session);
  var hash = crypto.createHash('sha256');
  hash.update(password + JSON.stringify(toSign));

  return hash.digest('base64');
}

function sign(password, session) {
  session._signature = getSignature(password, session);
  return session;
}
        ...
        request.auth.session = {
            set: function (session, value) {
                ...
                if (arguments.length > 1) {
                    ...
                    session[key] = value;
                    session = sign(settings.password, session);
    ...
    var scheme = {
        authenticate: function (request, reply) {
            ...
            var validate = function () {
                ...
                var noAuth = !session || 
                  session._signature !== getSignature(settings.password, session);
